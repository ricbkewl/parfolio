...
(function(){
  ...
  function mergeRow(row){
    ...
    const common={parfolioCatalogId:row.catalog_id,parfolioMappingClass:mapping,parfolioMappedHoleCount:Number(row.mapped_holes)||0,parfolioCaliforniaAudit:true,openGolfApiId:row.source_id||null,city:row.city||'',state:row.state_code||STATE,postal_code:row.postal_code||'',country:'United States',country_code:row.country_code||'US',address:row.address||'',website:row.website||'',catalog_point:location,par_total:Number(row.par)||null,course_type:row.course_type||'',sourceLicense:row.source_license||'ODbL-1.0',sourceAttribution:row.source_attribution||'',osmCourseUri:row.osm_course_uri||null,imageUrl:row.image_url||'',imageSourceUrl:row.image_source_url||'',imageLicense:row.image_license||'',imageAttribution:row.image_attribution||'',imageStatus:row.image_status||'satellite_fallback',imageReviewedAt:row.image_reviewed_at||null,catalogApproved:mapping!=='quarantined',catalogOnly:mapping!=='gps_ready'};
    if(idx>=0){
