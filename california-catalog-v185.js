/* ParFolio v185 — audited California catalog + validated GPS loader.
...
(function(){
  ...
  function mergeRow(row){
    ...
    const common={
      ...
      address:row.address||'',catalog_point:location,par_total:Number(row.par)||null,course_type:row.course_type||'',
      sourceLicense:row.source_license||'ODbL-1.0',sourceAttribution:row.source_attribution||'',osmCourseUri:row.osm_course_uri||null,
      catalogApproved:mapping!=='quarantined',catalogOnly:mapping!=='gps_ready'
    };
    if(idx>=0){
      const prior=courses[idx],existingMapped=actualMappedCount(prior);
      courses[idx]={...prior,...common,
      ...
      // Never downgrade or overwrite already-complete geometry with imported OSM.
  ...
  async function hydrateCourse(course){
    ...
    if(declared&&(existing>=declared||course.parfolioPreservedVerifiedGeometry)){
      hydrated.add(course.parfolioCatalogId);course.catalogOnly=false;return true;
    }
    ...
    const greens=rows.map((g,index)=>({
      ...
      aim1:point(g?.aim1?.lat,g?.aim1?.lng),aim2:point(g?.aim2?.lat,g?.aim2?.lng),
      front:point(g?.front?.lat,g?.front?.lng),center:point(g?.center?.lat,g?.center?.lng),back:point(g?.back?.lat,g?.back?.lng),
      route:g?.route||null,_review:'parfolio-california-osm-validated',_source:g?.source||'openstreetmap_qlever',_hole:Number(g?.hole)||index+1
    }));
    ...
    const pars=rows.map(g=>Number(g?.par));
    if(pars.length===holes&&pars.every(p=>Number.isFinite(p)&&p>=2&&p<=7))course.pars=pars;
    ...
    loading=(async()=>{
      ...
      try{
        ...
        for(const row of rows){
          ...
          if(row.mapping_class==='gps_ready')stats.gpsReady++;
          else if(row.mapping_class==='partial_gps')stats.partialGps++;
          else if(row.mapping_class==='course_located')stats.courseLocated++;
          else if(row.mapping_class==='location_pending')stats.locationPending++;
          else if(row.mapping_class==='quarantined')stats.quarantined++;
        }
        courses.sort((a,b)=>String(a.name||'').localeCompare(String(b.name||'')));
        try{localStorage.parfolioCourses=JSON.stringify(courses)}catch{}
        stats.loaded=true;stats.loadedAt=new Date().toISOString();window.PARFOLIO_CA_CATALOG=stats;
  ...
  // Make existing discovery/filter logic use audited classifications without
  // claiming status-only records are mapped.
  const priorMappedCount=typeof mappedCount==='function'?mappedCount:null;
  mappedCount=function(course){
    if(course?.parfolioCaliforniaAudit){
  ...
  // Hydrate only the chosen complete layout before existing round setup runs.
  const priorStartCourseFromLibrary=startCourseFromLibrary;
  startCourseFromLibrary=async function(index){
    const course=courses?.[index];
    if(course?.parfolioCaliforniaAudit&&course.parfolioMappingClass==='quarantined'){
      alert(`${course.name} has ambiguous or unsafe GPS geometry and is not published for GPS play. Scorecard use remains available while the map is reviewed.`);
    }
    if(course?.parfolioCaliforniaAudit&&course.parfolioMappingClass==='gps_ready'){
      try{await hydrateCourse(course)}catch(error){console.warn('California GPS hydration failed',error);alert('This course is listed as GPS Ready, but its validated hole payload could not be loaded. Please try again.');return;}
    }
    return priorStartCourseFromLibrary(index);
  };

  function auditedState(course){
    const m=course?.parfolioMappingClass;
    if(m==='gps_ready')return{key:'ready',label:'GPS Ready',short:'GPS Ready'};
    if(m==='partial_gps')return{key:'partial',label:'Partial GPS',short:'Partial GPS'};
    if(m==='course_located')return{key:'located',label:'Course Located',short:'Located'};
    if(m==='quarantined')return{key:'missing',label:'Quarantined',short:'Review'};
    if(m==='location_pending')return{key:'missing',label:'Location Pending',short:'Pending'};
    return null;
  }
  window.parfolioAuditedGpsState=auditedState;

  // Correct badges generated by the existing smart-search layer, including
  // Quarantined / Location Pending states that predate the five-state audit.
  function repairStatuses(root=document){
    root.querySelectorAll?.('.smart-course-row').forEach(row=>{
      ...
      const course=(courses||[]).find(c=>c.name===name),state=auditedState(course);if(!state)return;
      const badge=row.querySelector('.smart-gps-badge');if(badge){badge.className=`smart-gps-badge ${state.key}`;badge.textContent=`● ${state.label}`;}
    });
    root.querySelectorAll?.('.smart-course-suggestions button').forEach(button=>{
      const name=button.querySelector('b')?.textContent?.trim();const course=(courses||[]).find(c=>c.name===name),state=auditedState(course);if(!state)return;
      const badge=button.querySelector('.smart-search-status');if(badge){badge.className=`smart-search-status ${state.key}`;badge.setAttribute('aria-label',state.label);badge.innerHTML=`<i>◎</i>${state.short}`;}
    });
  }
  window.repairParFolioCaliforniaStatuses=repairStatuses;
  new MutationObserver(()=>repairStatuses()).observe(document.documentElement,{childList:true,subtree:true});
