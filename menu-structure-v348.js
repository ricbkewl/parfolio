/* ParFolio v348 — Settings menu Sponsor Business Center fix */
(function(){
  const LABELS={
  ...
  window.showParFolioSettingsMenu=function(){
    ...
    const L=labels(),active=!!s.sharedRoundId,overlay=document.createElement('div');overlay.className='app-menu-overlay';overlay.onclick=event=>{if(event.target===overlay)closeRoundQuickMenu()};
    const playingTee=active?`<button onclick="${closeAnd("showTeePicker()")}">${icon('◎')}<div>${esc(t('playingTee'))}<small>${esc(teeSetLabel())}</small></div></button>`:'';
    overlay.innerHTML=`<section class="app-side-menu menu-v204" role="dialog" aria-modal="true" aria-label="${esc(L.settings)}">
      ...
      ${adminRole==='super_admin'?'<button class="menu-sponsor-business-center" onclick="closeRoundQuickMenu();openSponsorBusinessCenter()">'+icon('◆')+'<div>Sponsor Business Center<small>Prospects · Campaigns · Revenue</small></div></button>':''}
    </section>`;
  ...
  showAppMenu=function(){
    ...
    overlay.innerHTML=`<section class="app-side-menu menu-v204" role="dialog" aria-modal="true" aria-label="${esc(t('menu'))}">
      ...
      ${section(L.settings)}
      <button onclick="closeRoundQuickMenu();showParFolioSettingsMenu()">${icon('⚙')}<div>${esc(L.settings.charAt(0)+L.settings.slice(1).toLowerCase())}<small>${esc(L.account)} · ${esc(menuExtraLabel('clubs'))} · ${esc(L.history)}</small></div></button>
    </section>`;
    document.body.appendChild(overlay);
  };
})();
