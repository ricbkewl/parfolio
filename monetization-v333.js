/* ParFolio v333 — monetization foundation: direct sponsors first, AdSense-ready, map-safe. */
(function(){
  const VERSION='333';
  ...
  function addSlot(placement,target,mode='append'){
    if(!target||target.querySelector?.('[data-pf-monetization-slot="'+placement+'"]'))return;
  ...
  function decorateSlots(){
    ...
    if(appNode.classList.contains('home-page')){
      ...
      if(hero)addSlot('home_banner',hero,'after');
    }
    ...
    if(view==='setup'){
      ...
      if(info)addSlot('course_preview',info,'after');
    }

    if(view==='scorecard'){
      addSlot(typeof s!=='undefined'&&s?.done?'round_complete':'scorecard',appNode,'append');
    }else if(typeof s!=='undefined'&&s?.done){
      addSlot('round_complete',appNode,'append');
    }

    /* Deliberately no in-round GPS placement in v333.
       The setting exists for future controlled testing, but the live map stays protected. */
  }
  ...
  function decorate(){
    requestAnimationFrame(()=>{decorateSlots();injectAdminEntry()});
  }
  ...
  function campaignRow(c){
    ...
    return '<article class="pf-campaign-row">'+
      '<div><small>'+safe(c.placement.replaceAll('_',' ').toUpperCase())+'</small><b>'+safe(c.sponsor_name)+' · '+safe(c.headline)+'</b><span>'+(c.active?(live?'LIVE':'SCHEDULED'):'OFF')+' · Priority '+safe(c.priority)+'</span></div>'+
  ...
  if(appNode){
    ...
    observer.observe(appNode,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
  }

  setTimeout(()=>loadMonetization(),0);
})();