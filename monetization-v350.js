/* ParFolio v350 — stable monetization renderer */
(function(){
  const VERSION='350';
  ...
  if(appNode){
    observer=new MutationObserver(mutations=>{
      if(decoratingSlots)return;
      const meaningful=mutations.some(m=>{
        const target=m.target?.nodeType===1?m.target:null;
        if(target?.closest?.('[data-pf-monetization-slot]'))return false;
        const nodes=[...m.addedNodes,...m.removedNodes].filter(n=>n?.nodeType===1);
        if(nodes.length&&nodes.every(n=>n.matches?.('[data-pf-monetization-slot]')||n.closest?.('[data-pf-monetization-slot]')))return false;
        return true;
      });
      if(meaningful)decorate();
    });
