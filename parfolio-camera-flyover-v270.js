/* ParFolio v286 — bounded forward-facing maximum-tilt hole camera.
   The v269 Google-only renderer remains the source of truth.
   No flyover animation. No camera travel. No projection feedback loops.
   On supported Google Vector maps, orient the current hole with tee toward 6 o'clock,
   green center toward 12 o'clock, apply one safe zoom boost, and use the maximum
   practical static tilt on every mapped hole from every course source.
   If vector rendering is unavailable, keep the working Google map unchanged. */
(function(){
  const MAX_TILT=67.5;
  const SAFE_ZOOM_BOOST=.55;
  const MIN_ZOOM=15.5;
  const MAX_ZOOM=19.5;
  ...
  function cameraFor(green,raw){
    ...
    return{
      ...
      zoom:clamp(fitted+SAFE_ZOOM_BOOST,MIN_ZOOM,MAX_ZOOM),
  ...
  function applyCurrentCamera(force=false){
    ...
    try{
      ...
      if(host){host.dataset.forwardBearing=String(camera.heading);host.dataset.cameraRule='all-courses-single-pass';host.dataset.cameraTilt=String(camera.tilt)}
      applied.set(raw,sig);record('BOUNDED_CAMERA_APPLIED',`hole ${s?.hole||1} · one camera move · zoom +${SAFE_ZOOM_BOOST}`);return true;
  ...
  if(typeof priorUpdate==='function')window.updateGoogleRoundHole=function(){
    ...
    try{
      if(inlineHoleMap?.provider==='google'&&inlineHoleMap.raw){
        armVectorOnce(inlineHoleMap.raw);
        setTimeout(()=>applyCurrentCamera(false),90);
      }
  ...
  record('BOUNDED_CAMERA_V286_READY',`${MAX_TILT} degrees · single pass · all mapped holes`);
})();
