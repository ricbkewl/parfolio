(() => {
  ...
  async function loadClubBag(){
    ...
    if(error)return [];
    return (data||[]).filter(club=>Number.isFinite(Number(club.carry_yards))).map(club=>({name:club.club_name,carry:Number(club.carry_yards)}));
  }
  ...
  async function openLiveRound(round){
    ...
    let holeIndex = 0;
    let map, routeLine, gpsMarker, watchId, gpsPoint, activePoints=[], plannerMarker, plannerHitLine, plannerGoLine, plannerHitLabel, plannerGoLabel, flyoverFrame, cameraAuditToken=0, resizeTimer, viewportHandler;
    const plannerAims = {};
    let mapMarkers = [];
    ...
    open(`<div class="live-round-shell">
      ...
      <div class="pf-play-menu-overlay" id="pfPlayMenuOverlay" hidden><nav class="pf-play-menu"><header><strong>ParFolio</strong><button type="button" id="closePfPlayMenu">×</button></header><button type="button" data-pf-menu="home"><span>⌂</span>Home</button><button type="button" data-pf-menu="tee"><span>⛳</span>Playing Tee<small>${esc(round?.playing_tee||'Not selected')}</small></button><button type="button" data-pf-menu="scorecard"><span>▦</span>Round / Scorecard</button><button type="button" data-pf-menu="code"><span>▣</span>Show Code / QR</button><div class="pf-code-panel" id="pfCodePanel" hidden><small>ROUND CODE</small><b>${esc(round?.round_code||'—')}</b><button type="button" id="copyPfCode">Copy Code</button></div><button type="button" data-pf-menu="courses"><span>⌖</span>Courses / Players</button><button type="button" data-pf-menu="language"><span>◎</span>Language<small id="pfLanguageLabel">English</small></button><div class="pf-language-panel" id="pfLanguagePanel" hidden>${[['en','English'],['es','Español'],['zh','中文'],['id','Bahasa Indonesia'],['hi','हिन्दी'],['fr','Français']].map(([code,label])=>`<button type="button" data-language="${code}"><span>✓</span>${label}</button>`).join('')}</div><button type="button" data-pf-menu="account"><span>◎</span>My Account</button><button type="button" data-pf-menu="guide"><span>?</span>App Guide</button><button type="button" data-pf-menu="about"><span>PF</span>About ParFolio</button></nav></div>
    </div>`);
    ...
    const routeProjection = (point,start,end) => {const scale=Math.cos((start.lat+end.lat)*Math.PI/360),ax=start.lng*scale,ay=start.lat,bx=end.lng*scale,by=end.lat,px=point.lng*scale,py=point.lat,dx=bx-ax,dy=by-ay,length=dx*dx+dy*dy;return length?((px-ax)*dx+(py-ay)*dy)/length:1};
    const activeSegment = (origin,points) => {for(let index=1;index<points.length;index++){const close=yardsBetween(origin,points[index])<45,passed=routeProjection(origin,points[index-1],points[index])>.88;if(index===points.length-1||(!close&&!passed))return index}return Math.max(1,points.length-1)};
    const updatePlanner = (points=activePoints) => {
      ...
      const segmentIndex=activeSegment(origin,points),tail=points.slice(segmentIndex+1),remaining=[aim,...tail,...(tail.at(-1)===points.at(-1)?[]:[points.at(-1)])];
    ...
    document.querySelectorAll('[data-pf-menu]').forEach(button=>button.addEventListener('click',()=>{const action=button.dataset.pfMenu;if(action==='home'){dialog.close();return}if(action==='tee'){const tee=prompt('Playing Tee:',round?.playing_tee||'');if(tee){round.playing_tee=tee.trim();button.querySelector('small').textContent=round.playing_tee;drawHole(true)}return}if(action==='scorecard'){menuOverlay.hidden=true;renderScore();document.getElementById('liveScorecardSheet').hidden=false;return}if(action==='code'){document.getElementById('pfCodePanel').hidden=!document.getElementById('pfCodePanel').hidden;return}if(action==='language'){languagePanel.hidden=!languagePanel.hidden;return}menuOverlay.hidden=true;dialog.close();if(action==='courses'){document.querySelector('[data-screen="courses"]')?.click();return}showContent(action==='account'?'profile':action)}));
