...
const queries=[
  ['Swing',`${TRUSTED} golf swing instruction drill when:14d`],
  ['Short Game',`${TRUSTED} golf putting chipping bunker wedge tips when:14d`],
  ['Courses',`${TRUSTED} best golf courses ${year} rankings public course when:60d`],
  ['Trends',`${TRUSTED} golf equipment driver irons ball technology gear when:21d`],
...
const CATEGORY_MATCH={
  ...
  Courses:/\b(golf course|courses|course ranking|ranked|best public|best private|top 100|top 50|architecture|golf club|golf resort|destination)\b/i,
  Trends:/\b(driver|iron|irons|wedge|putter|golf ball|equipment|gear|launch monitor|rangefinder|technology|club fitting|golf club|shaft)\b/i,
  Travel:/\b(golf travel|golf trip|destination|resort|stay.?and.?play|golf vacation|golf holiday|golf tourism)\b/i
};
...
const cleanSummary=(description,title,source)=>{let t=strip(description).replace(new RegExp(String(source||'').replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'$','i'),'').trim();if(canonicalTitle(t).startsWith(canonicalTitle(title)))t='';return t.length>180?t.slice(0,177)+'…':t};
  ...
  for(const [category,q] of queries){
    ...
    try{
      ...
      for(const block of xml.match(/<item>[\s\S]*?<\/item>/gi)||[]){
        const source=strip(tag(block,'source'))||'Golf News',title=cleanTitle(tag(block,'title'),source),link=strip(tag(block,'link')),description=tag(block,'description'),publishedAt=tag(block,'pubDate'),text=title+' '+strip(description);
        if(!title||!link||BLOCKED.test(text)||!CATEGORY_MATCH[category].test(text))continue;
        items.push({type:'article',category,title,summary:cleanSummary(description,title,source),source,url:link,publishedAt:new Date(publishedAt||Date.now()).toISOString(),thumbnail:imageFrom(description)});
      }
