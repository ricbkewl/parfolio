/* ParFolio v345 — Sponsor Business Center */
(function(){
  ...
  function inject(){
    if(!isSuper())return;
    const settings=document.querySelector('.pf-page-settings');
    if(!settings||settings.querySelector('[data-pf-business-center]'))return;
    ...
    const mon=settings.querySelector('[data-pf-monetization-admin]');
    if(mon)settings.insertBefore(button,mon);
    else{
      const danger=settings.querySelector('.danger');
      if(danger)settings.insertBefore(button,danger);else settings.appendChild(button);
    }
  }
